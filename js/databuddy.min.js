(() => {
  ...
  const l = class {
    ...
    async send(event) {
      ...
      const fetchOptions = { keepalive: true };
    ...
    async sendExitEventImmediately(exitEvent) {
      try {
        ...
        return this.api.fetch("/", exitEvent, { keepalive: true });
      } catch (e) {
        return null;
      }
    }
  ...
  const d = class extends l {
    constructor(t) {
      super({ sdk: "web", sdkVersion: "1.0.2", ...t });
      if (this.isServer()) return;
    ...
    trackScreenViews() {
      ...
      const i = () =>
        this.debounce(() => {
          ...
          this.setGlobalProperties({ referrer: previous_path });
          this.isInternalNavigation = true;
          this.screenView();
        }, 50);
  ...
  function initializeDatabuddy() {
    ...
    function getConfig() {
      ...
      const config = { ...globalConfig, ...urlParams, ...dataAttributes };
    ...
    function init() {
      ...
      if (!clientId) return;
      window.databuddy = new d({ ...config, clientId: clientId });
      window.db = {
