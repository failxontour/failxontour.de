(()=>{
    ...
    const c = class {
        constructor(config){
            ...
            for (const [key, value] of Object.entries(headers))if (typeof value === "function" || value && typeof value.then === "function") this.dynamicHeaderFns[key] = value;
            else this.staticHeaders[key] = value;
            this.maxRetries = config.maxRetries ?? 3;
            this.initialRetryDelay = config.initialRetryDelay ?? 500;
        }
        async resolveHeaders() {
            const dynamicEntries = await Promise.all(Object.entries(this.dynamicHeaderFns).map(async ([key, fn])=>[
                    key,
        ...
        async post(url, data, options = {}, retryCount = 0) {
            try {
                ...
                if (response.status !== 200 && response.status !== 202) {
                    if ((response.status >= 500 && response.status < 600 || response.status === 429) && retryCount < this.maxRetries) {
    ...
    const l = class {
        ...
        init() {
            ...
            if (this.options.trackSessions) {
                ...
                this.sessionStartTime = this.getSessionStartTime();
                this.lastActivityTime = Date.now();
            } else this.anonymousId = this.getOrCreateAnonymousId();
            ...
            if (this.options.trackInteractions) for (const eventType of interactionEvents)window.addEventListener(eventType, ()=>{
                ...
                const handler = ()=>{
                    this.interactionCount++;
                    for (const eventType of interactionEvents)window.removeEventListener(eventType, handler);
                };
                for (const eventType of interactionEvents)window.addEventListener(eventType, handler, {
                    passive: true
                });
            }
            if (this.options.trackEngagement) {
                ...
                this.interactionCount = 0;
                this.hasExitIntent = false;
                if (this.options.trackExitIntent) document.addEventListener("mouseleave", (e)=>{
                    if (e.clientY <= 0) this.hasExitIntent = true;
                });
            }
            if (this.options.trackErrors) window.addEventListener("error", (event)=>{
                this.trackError({
                    timestamp: Date.now(),
        ...
        async send(event) {
            ...
            if (this.options.disabled || this.options.filter && !this.options.filter(eventData)) return Promise.resolve();
            if (this.options.enableBatching && !event.isForceSend) return this.addToBatch(eventData);
            const fetchOptions = {
                keepalive: true
            };
        ...
        addToBatch(event) {
            ...
            if (this.batchTimer === null) this.batchTimer = setTimeout(()=>this.flushBatch(), this.options.batchTimeout);
            if (this.batchQueue.length >= this.options.batchSize) this.flushBatch();
        ...
        async flushBatch() {
            ...
            try {
                ...
                if (isNetworkError) for (const event of batchEvents){
                    const originalEvent = {
                        type: "track",
                        payload: event,
                        isForceSend: true
        ...
        async track(eventName, properties) {
            ...
            if (this.options.samplingRate < 1) {
                ...
                if (samplingValue > this.options.samplingRate) return {
                    sampled: false
                };
            }
            let finalProperties;
            if (properties === undefined || properties === null) finalProperties = {};
            else if (typeof properties === "object") finalProperties = properties;
            else finalProperties = {
                value: properties
            };
            const baseContext = this.getBaseContext();
            let performanceData = {};
            if ((eventName === "screen_view" || eventName === "page_view") && !this.isServer() && this.options.trackPerformance) {
                performanceData = this.collectNavigationTiming();
                if (this.options.trackWebVitals) this.initWebVitalsObservers(eventName);
            }
            const payload = {
                type: "track",
                payload: {
            ...
            if (this.options.enableBatching) return this.send(payload);
            try {
        ...
        async sendBeacon(event) {
            ...
            try {
                const eventData = event.type === "track" && event.payload ? event.payload : event;
                if (this.options.disabled || this.options.filter && !this.options.filter(eventData)) return null;
        ...
        clear() {
            ...
            if (typeof window !== "undefined" && window.localStorage) localStorage.setItem("did", this.anonymousId);
        ...
        collectNavigationTiming() {
            ...
            const clampTime = (v)=>typeof v === "number" ? Math.min(6e4, Math.max(0, v)) : v;
            try {
        ...
        setupExitTracking() {
            ...
            if (!("onpagehide" in window)) window.addEventListener("beforeunload", exitHandler);
        }
        trackExitData() {
            if (this.isServer()) return;
            const baseContext = this.getBaseContext();
            const exitEventId = `exit_${this.sessionId}_${btoa(window.location.pathname)}_${this.pageEngagementStart}`;
            const page_count = Math.min(1e4, this.pageCount);
            const interaction_count = Math.min(1e4, this.interactionCount);
            const time_on_page = Math.min(86400, Math.round((Date.now() - this.pageEngagementStart) / 1e3));
            const exitEvent = {
                type: "track",
                payload: {
                    eventId: exitEventId,
        ...
        cleanupWebVitals() {
            if (this.webVitalObservers) {
                for (const o of this.webVitalObservers)try {
                    o.disconnect();
                } catch (e) {
                    console.error(e);
                }
        ...
        initWebVitalsObservers(eventName) {
            if (this.isServer() || !this.options.trackWebVitals || typeof window.performance === "undefined" || typeof PerformanceObserver === "undefined") return;
            try {
                const metrics = {
                    fcp: null,
                    lcp: null,
                ...
                let reported = false;
                const clamp = (v)=>typeof v === "number" ? Math.min(6e4, Math.max(0, v)) : v;
                const report = ()=>{
                ...
                observe("paint", (entries)=>{
                    for (const entry of entries)if (entry.name === "first-contentful-paint" && !metrics.fcp) metrics.fcp = Math.round(entry.startTime);
                });
                ...
                observe("layout-shift", (entries)=>{
                    for (const entry of entries)if (!entry.hadRecentInput) metrics.cls += entry.value;
                });
                observe("first-input", (entries)=>{
                    const entry = entries[0];
                    if (entry && !metrics.fid) metrics.fid = Math.round(entry.processingStart - entry.startTime);
                });
                observe("event", (entries)=>{
                    for (const entry of entries)if (entry.interactionId && entry.duration > (metrics.inp || 0)) metrics.inp = Math.round(entry.duration);
                });
                ...
                this.webVitalsReportTimeoutId = setTimeout(report, 1e4);
            } catch (e) {}
        }
        getConnectionInfo() {
        ...
        getBaseContext() {
            ...
            if (typeof width !== "number" || typeof height !== "number" || width < 240 || width > 1e4 || height < 240 || height > 1e4) {
                width = null;
                height = null;
            }
            ...
            if (typeof screenWidth !== "number" || typeof screenHeight !== "number" || screenWidth < 240 || screenWidth > 1e4 || screenHeight < 240 || screenHeight > 1e4) {
                screenWidth = null;
                screenHeight = null;
            }
        ...
        async trackError(errorData) {
            ...
            if (this.options.enableBatching) return this.send(errorEvent);
            try {
        ...
        async trackWebVitals(vitalsData) {
            ...
            if (this.options.enableBatching) return this.send(webVitalsEvent);
            try {
    ...
    const d = class extends l {
        constructor(t){
            ...
            if (this.options.trackOutgoingLinks) this.trackOutgoingLinks();
            if (this.options.trackAttributes) this.trackAttributes();
            this.init();
        }
        debounce(t, r) {
            ...
            this.isServer() || document.addEventListener("click", (t)=>{
                ...
                if (i && r) {
                    ...
                    if (n) try {
                        ...
                        if (isOutgoing) this.track("link_out", {
                            href: n,
                            text: i.innerText || i.getAttribute("title") || r.getAttribute("alt")
                        });
                    } catch (e) {}
                }
            });
        ...
        trackAttributes() {
            this.isServer() || document.addEventListener("click", (t)=>{
                ...
                if (s) {
                    const o = {};
                    for (const p of s.attributes)if (p.name.startsWith("data-") && p.name !== "data-track") o[h(p.name.replace(/^data-/, ""))] = p.value;
        ...
        screenView(t, r) {
            ...
            if (this.lastPath !== i) {
                if (this.options.trackWebVitals) this.cleanupWebVitals();
                this.lastPath = i;
                this.pageCount++;
                this.isInternalNavigation = false;
                const pageData = {
                    page_count: Math.min(1e4, this.pageCount),
                    ...n ?? {}
                };
    ...
    function initializeDatabuddy() {
        ...
        try {
            if (localStorage.getItem("databuddy_opt_out") === "true" || localStorage.getItem("databuddy_disabled") === "true" || window.databuddyOptedOut === true || window.databuddyDisabled === true) {
                window.databuddy = {
                    track: ()=>{},
                    screenView: ()=>{},
                    clear: ()=>{},
        ...
        function getConfig() {
            ...
            if (!currentScript) return globalConfig;
            const dataAttributes = {};
            for (const attr of currentScript.attributes)if (attr.name.startsWith("data-")) {
                const key = attr.name.substring(5).replace(/-([a-z])/g, (_, letter)=>letter.toUpperCase());
                const value = attr.value;
                if (value === "true") dataAttributes[key] = true;
                else if (value === "false") dataAttributes[key] = false;
                else if (/^\d+$/.test(value)) dataAttributes[key] = Number(value);
                else dataAttributes[key] = value;
            }
            const urlParams = {};
            try {
                ...
                params.forEach((value, key)=>{
                    ...
                    else if (/^\d+$/.test(value)) urlParams[key] = Number(value);
            ...
            if (config.maxRetries !== undefined && config.maxRetries < 0) config.maxRetries = 0;
            if (config.initialRetryDelay !== undefined) {
                if (config.initialRetryDelay < 50) config.initialRetryDelay = 50;
                if (config.initialRetryDelay > 1e4) config.initialRetryDelay = 1e4;
            }
            ...
            if (config.batchTimeout !== undefined) {
                ...
                if (config.batchTimeout > 3e4) config.batchTimeout = 3e4;
            }
            if (config.apiUrl) try {
                new URL(config.apiUrl);
            } catch (e) {
                config.apiUrl = "https://basket.databuddy.cc";
            }
            else config.apiUrl = "https://basket.databuddy.cc";
            return config;
        }
        function getClientId(config) {
            if (config.clientId) return config.clientId;
            if (currentScript?.getAttribute("data-client-id")) return currentScript.getAttribute("data-client-id");
            return null;
        }
        function init() {
    ...
    if (typeof window !== "undefined") {
        ...
        try {
            const urlParams = new URLSearchParams(window.location.search);
            if (urlParams.get("databuddy_opt_out") === "true" || urlParams.get("no_tracking") === "true") window.databuddyOptOut();
        } catch (e) {}
    } else if (typeof exports === "object") module.exports = d;
})();

//# sourceMappingURL=failxontour.de.df744968.js.map
