<?php
	...
	$encryption_key		= 'Her0ricTurtle';			// Unique 8 to 32 character encryption key
	$notification_email	= 'webmaster@failxontour.de';	// E-mail for notification of new comments
	$admin_nickname		= 'failx';			// Nickname with admin rights (must be title-cased)
	$admin_password		= '*kpj+!5Ds=$c~J=YqM';			// Password to gain admin rights
	...
	if (isset($_GET['source']) and basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
		...
		echo "\t" . '$encryption_key' . "\t\t" . '= \'Her0ricTurtle\';' . "\t\t\t" . '// Unique 8 to 32 character encryption key' . PHP_EOL;
		echo "\t" . '$notification_email' . "\t" . '= \'webmaster@failxontour.de\';' . "\t" . '// E-mail for notification of new comments' . PHP_EOL;
		echo "\t" . '$admin_nickname' . "\t\t" . '= \'failx\';' . "\t\t\t" . '// Nickname with admin rights (must be title-cased)' . PHP_EOL;
		echo "\t" . '$admin_password' . "\t\t" . '= \'*kpj+!5Ds=$c~J=YqM\';' . "\t\t\t" . '// Password to gain admin rights' . PHP_EOL . PHP_EOL;
		echo '?>';
	}
